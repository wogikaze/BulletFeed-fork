...
HOLD_INDEX = (
    ROOT / "backend" / "tests" / "gold" / "delta_adversarial" / "v01" / "blind" / "index.json"
)
...
def main() -> None:
    ...
    if violations:
        raise SystemExit(
            "delta-adversarial Gold leakage into production code:\n" + "\n".join(violations)
        )
    print(f"delta-adversarial Gold isolation OK: {len(forbidden)} forbidden tokens scanned")
