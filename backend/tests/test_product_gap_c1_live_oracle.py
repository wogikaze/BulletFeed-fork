...
async def test_live_g3_is_dev_only_and_records_parity(monkeypatch) -> None:
    ...
    async def fake_fetch(_settings, _url):
        return body, "https://blog.example.com/feed.xml", "application/xml"
    ...
    assert report["family_regression_measured"] is True
...
@pytest.mark.asyncio
async def test_live_g4_keeps_longitudinal_unmeasured(monkeypatch) -> None:
    ...
    assert report["sample_complete"] is False
    with pytest.raises(ValueError, match="dev-only"):
        await body.measure_live_g4(GOLD_V2, split="blind", max_items=0, delay_seconds=0)
