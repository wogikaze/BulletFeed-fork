...
def test_history_rich_personas_stale_display_does_not_hard_hide(database) -> None:
    personas = [row for row in load_persona_manifest(_MANIFEST) if row.cohort == "history_rich"]
    assert len(personas) >= 15
    assert {row.language for row in personas} >= {"ja", "en", "mixed"}
    result = StatuspagePipeline(database).ingest_summary(
        page_id="abcd1234",
        summary=_summary(),
        retrieved_at="2026-08-22T00:11:00Z",
    ...
    for persona in personas:
        ...
        with database.connect() as connection:
            connection.execute(
                ...
                WHERE user_id = ? AND kind = ?
                """,
                (stale_at, user_id, KIND_DISPLAYED),
            )
            claim_id = connection.execute(
                """
                SELECT claim_id FROM user_knowledge_evidence
        ...
        assert derived.state == "unknown", user_id
        after, _ = store.list_feed(user_id, relation=None, item_status=None, cursor=None, limit=50)
        assert {item.id for item in after} == {item.id for item in items}
        detail = EventStore(database).get_event(user_id, event_id, None)
        assert detail.unknown_facts, user_id
