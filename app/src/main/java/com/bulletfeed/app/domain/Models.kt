...
enum class Importance(val label: String, val color: Color) {
...
enum class Relation(val label: String, val color: Color) {
...
enum class FeedFilter(val label: String) { ALL("すべて"), DIRECT("直接影響"), ADJACENT("近接影響"), REFERENCE("参考") }
enum class AppTab(val label: String, val symbol: String) { FEED("フィード", "●"), SEARCH("検索", "⌕"), TOPICS("テーマ", "◎"), SETTINGS("設定", "◌") }
...
data class Source(val publisher: String, val title: String, val evidence: String)
